<script lang="ts">
	...
	async function createMetakey() {
		...
		try {
			...
			const metakey = `0x${(signature as string).slice(2).substring(0, 64)}`;
	...
	{:else}
		...
		<CardContent class="space-y-4">
			<Alert>
				...
				<AlertDescription>
					Click the button below to sign a message with your wallet. This
					will generate your unique MetaKey.
				</AlertDescription>
			</Alert>
